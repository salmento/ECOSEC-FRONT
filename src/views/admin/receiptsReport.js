...
const Receipt = () => {
    ...
    const listreceiptNotPayed = async () => {

        try {
            const response = await axios.get(`receipt/listreceipt?initialDate=${initialDate}&finalDate=${finalDate}&addressId=${addressId}`,
                {
                    ...
                    <Col className="order-xl-1 mb-5 mt-7 mb-xl-0" >
                        <Card className="card-profile shadow">
                            ...
                            <CardHeader className="text-center border-0 pt-2 pt-md-4 pb-0 pb-md-0">
                                <h3 className="mb-0 text-default">Recibos gerados</h3>
                            </CardHeader>
                            <CardBody>
                                <Form className="pl-4 font-weight-bold text-uppercase">
                                    <Row >
                                        ...
                                        <Col md="4">
                                            <FormGroup>
                                                <Button onClick={listreceiptNotPayed} block className="mt-4 p-3 btn btn-success">Pesquisar</Button>
                                            </FormGroup>
                                        </Col>
                                    </Row>
                                            ...
                                            >
                                                <thead className="text-white bg-info p-1 h1">
                                                    <tr>
                                                        ...
                                                        <th className="p-1  pl-2" scope="col">T. Pagamento</th>
                                                ...
                                                <tbody className=" p-0 text-default">
                                                    {receipt?.map((receipt, index) => (
                                                        <tr key={index} >
                                                            <td className="p-1  pl-2">{receipt?.clientId}</td>
                                                            <td className="p-1  pl-2">{receipt?.clientName}</td>
                                                            <td className="p-1  pl-2">{receipt?.clientSurname}</td>
                                                            <td className="p-1  pl-2">{receipt?.clientPhone}</td>
                                                            <td className="p-1  pl-2">{receipt?.receiptRef}</td>
                                                            <td className="p-1  pl-2">{receipt?.orderRef}</td>
                                                            <td className="p-1  pl-2">{receipt?.payedMoney}</td>
                                                            <td className="p-1  pl-2">{receipt?.totalToPay}</td>
                                                            <td className="p-1  pl-2">{moment(receipt?.createdAt).format("L")}</td>
                                                            <td className="p-1  pl-2">{receipt?.paymentStatus}</td>
                                                            <td className="p-1  pl-2">{receipt?.vendorName} {receipt?.vendorSurname}</td>
                                                        </tr>
                                                    ))}
