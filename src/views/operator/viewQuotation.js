...
const Quotations = () => {
  ...
  const [quotationRef, setQuotationRef] = useState(0)
  ...
  useEffect(() => {
    const order = async () => {
      ...
      try {
        if (clientId) {
          const response = await axios.get(`quotation/index/${clientId}`,
            {
              headers: { 'accesstoken': `${accessToken}` },
            }
          );
  ...
  const handleEdit = (quotation) => {
    setOrders(JSON.parse(quotation?.order))
    setTotal(quotation?.totalToPay)
    setQuotationRef(quotation?.quotationRef)
  }
        ...
        <Col className="order-xl-1 mb-5 mt-7 mb-xl-0 " xl="6" >
          <Card className="card-profile shadow">
            ...
            <CardHeader className="text-center border-0 pt-2 pt-md-4 pb-0 pb-md-0">
              <h3 className="mb-0 text-default">Visualizar Cotacao dum cliente</h3>
            </CardHeader>
            <CardBody>
              <Form className="pl-4 font-weight-bold text-uppercase">
                ...
                <Row >
                  ...
                  <Col className="mt-4 pt-2 pb-2 " >
                    ...
                    <Button onClick={handlePrinter} block disabled={total ? false : true}>Imprimir Cotacao</Button>
                  </Col>
                  ...
                  <Col md="12">
                    ...
                    >
                      <thead className="text-white p-1 h1">
                        <tr>
                          <th className="p-1" scope="col">N. Cotacao</th>
                      ...
                      <tbody className="text-white p-1">
                        ...
                        {Quotations?.map((quotation, index) => (
                          <tr key={index} value={quotation} >
                            <td className="pl-1 p-0">{quotation?.quotationRef}</td>
                            <td className="pl-1 p-0">{quotation?.totalToPay}</td>
                            <td className="pl-1 p-0">{quotation?.createdAt ? new Date(quotation?.createdAt).toISOString(0, 10).substring(0, 10) : ""}</td>
                            <td className="pl-1 p-0">{quotation?.clientId}</td>
                            <td className="pl-1 p-0">{quotation?.clientName}</td>
                            <td className="pl-1 p-0">{quotation?.clientSurname}</td>
                            <td className="pl-1 p-0"><Button className="btn btn-white p-1" onClick={(e) => { handleEdit(quotation) }}>Selecionar</Button></td>
        ...
        <Col className="order-xl-1 text-darker" xl="6" id="print">
          <div ref={printRef}>
            ...
            <Card className={isPrint ? "" : "mt-7"}>
              ...
              <h3 className="m-0  p-0   pl-2 text-darker font-weight-bolder text-uppercase">Cotacao: {quotationRef}</h3>
